...
- Schema v10 installs database guards so older Workers fail closed for evidence/admin writes rather than approving or overwriting state they cannot understand. Capability revision 5 binds evidence retries to the exact loaded vendor, locks the captured snapshot through preflight and mutation, requires the client to re-prove that identity after every outcome, and binds the revision marker to the write so mixed Worker/client traffic cannot pass a check on one version and mutate through another. Evidence API checks also use the normalized vendor id as Cloudflare's gradual-deployment affinity key, keeping compatibility, context, mutation, and confirmation reads on one Worker version during a split. That client header does not pin initial HTML, hashed assets, or an older client that predates the contract; on the current `workers.dev` origin production therefore tests a new version at zero traffic with an explicit override and then cuts over atomically rather than exposing users to a percentage split. The summary-contract version likewise makes mixed generations pause safely until rollout convergence. Information requests are visible only in the product; this storage/API contract does not send or promise email, SMS, push, or another notification.
- Auction creation and bid acceptance require/replay an `Idempotency-Key` result. Before the first request write, the browser stores the exact payload, key, publish-time vendor display context, original timestamp, and owning user id in tab-scoped session storage. The create request also carries that expected user id for an atomic comparison with the authenticated server identity, so an account switch between client preflight and mutation fails without creating a request. A network-ambiguous publish restores the locked review only after re-proving the same account; unavailable storage fails before the write and retries never extend the server's 24-hour key window. A definitive rejection durably marks the protected record non-replayable, and editable state returns only after an explicit same-owner unlock.
- Request coverage counts only approved vendors with an active linked owner and an exact category/service-area match, and excludes a request owner's own vendor record. The public preflight is point-in-time guidance, the create response preserves its retry-stable snapshot, and owner dashboard reads recompute current coverage only while the open response window remains active. Zero is a valid result: the request may remain open, but the API and client do not promise a response or an unavailable notification channel.
- A preferred vendor is attached only when the vendor remains approved, is not owned by the requester, and matches the brief's category and city at the atomic create boundary. Other matched vendors cannot see that preference.
...
- `/api/v1/auth/*`
- `/api/v1/catalog/categories`, `/api/v1/catalog/coverage`, `/api/v1/catalog/vendors`, `/api/v1/vendors`, `/api/v1/vendors/onboarding`, and `/api/v1/vendors/onboarding/evidence`
...
- Hosted production promotion is disabled. The checked-in workflow is a read-only validation that intentionally fails after exact-main gates, bundle dry-runs, and a smoke of default production traffic; it has no production environment or Cloudflare credentials and cannot mutate traffic or the `production` tag. Production remains an authenticated local release from a clean merged-SHA checkout. Re-enabling hosted mutation requires a reviewed REST-based finalizer plus an `always()` reconciler whose write-token jobs use no checkout, package install, or third-party executable, persist immutable recovery inputs before mutation, reverse ambiguous traffic/tag compare-and-swaps, and preserve unrelated state.
