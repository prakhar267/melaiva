...
- [ ] Marketplace-to-request handoff preserves only a supported service, exact supported city, future ISO date, and bounded guest estimate; coverage preflight, create-time snapshot, and current dashboard count agree on approved category/service-area eligibility and exclude the requester's own vendor record.
- [ ] Zero, positive, unavailable, changed-after-publish, and ambiguous-publish coverage states never promise availability, an offer, email, push, or coverage notification; exact-payload retries remain account-bound across reload, focus, and account switches, preserve their first-attempt timestamp, fail before mutation when durable tab storage is unavailable, and do not create duplicate requests.
