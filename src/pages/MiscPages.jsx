    ...
    <div className="auth-page page-surface">
      <div className="shell auth-page__layout">
        ...
        <section className="auth-page__panel">
          <AuthPanel onSuccess={(mode) => { notify({ title: mode === "login" ? "Welcome back" : "Your account is ready", message: "Opening your planning space now." }); navigate("/dashboard"); }} />
        </section>
      </div>
    </div>
  );
...
const privacySections = [
  ["Information you give us", "We collect the details you enter when you create an account, build a celebration brief, request an offer, contact a partner or apply to join the vendor network. This can include names, email addresses, phone numbers, dates, locations, guest estimates, budgets and planning preferences."],
  ["How we use information", "We use this information to operate Melaiva, match briefs with suitable partners, produce requested planning assistance, secure accounts, provide support, improve service quality and meet legal obligations. We do not sell personal information."],
  ["When information is shared", "Relevant brief details may be shared with eligible partners so they can prepare an offer. Your direct contact details remain private until the product flow or your explicit action indicates that a connection should be made. We may also use vetted infrastructure providers that process information on our instructions."],
  ...
  ["Storage and security", "We apply technical and organisational safeguards appropriate to the nature of the information. No internet service can promise absolute security. We retain information only as long as needed for the purposes described here, legitimate business records and applicable law."],
...
export function LegalPage({ type }) {
  ...
  return (
    <div className="legal-page page-surface">
      <header className="legal-header"><div className="shell"><Link className="back-link" to="/"><ArrowLeft size={15} /> Back to Melaiva</Link><div className="eyebrow">Trust & transparency</div><h1>{privacy ? "Privacy policy" : "Terms of service"}</h1><p>Effective 16 August 2026 · Plain-language product policy</p></div></header>
