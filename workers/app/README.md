...
The operator UI lives at `/admin/vendors`. Queue and history reads remain server-authorized and never fall back to demo data. Review mutations require an `Idempotency-Key`, the exact `expectedStatus` and `expectedRevision` being reviewed, plus a 10–1,000 character internal rationale. The allowed lifecycle is `pending → approved | rejected`, `approved → suspended`, `suspended → approved`, and `rejected → pending`; withdrawn offers and unavailable invitations are never resurrected by restoration. Optional vendor evidence websites must be credential-free public HTTPS destinations; explicit local, private, and reserved hosts are rejected and legacy unsafe links are not opened by the operator UI.
