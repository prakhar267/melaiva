...
- Request coverage counts only approved vendor profiles with an active linked owner and an exact category/service-area match, excluding a requester's own vendor record. The public preflight is point-in-time guidance, the create response keeps a retry-stable snapshot, and owner-list reads recompute coverage only while the request is open and its response window has not ended. Zero is valid and never promises a future response or notification. Preferred invitations additionally recheck non-self active ownership and matching eligibility inside the create batch, so owner deletion or suspension races fail without an auction, invite, or idempotency record.
...
- Auction creation and bid acceptance honor `Idempotency-Key` (8-128 safe characters) and replay the original result for retry-safe clients. The request builder stores an in-flight exact payload, key, original timestamp, preferred-vendor display context, and owning user id in tab-scoped session storage before the first write. Create requests may send `X-Melaiva-Expected-User-Id`; the Worker compares it with the authenticated user before idempotency lookup or mutation, closing account-switch races. Recovery stays hidden until the same owner is verified, does not extend the 24-hour key window, and fails before mutation when durable tab storage is unavailable.
